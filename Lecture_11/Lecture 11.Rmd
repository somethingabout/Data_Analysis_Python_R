...
We'll learn to transform data with pivot_longer and pivot_wider
...
Let's import the dataset that we'll use today, growth rate by country and per capita income in us dollars, 
```{r}
...
We can transform the data into long format with the function
```{r}
...
Error in str_separate(value, into = into, sep = sep, convert = convert,  : 
...
You can also call each of the columsn within a datafram like this:
...
We can view some of the results in the stored item on the left. Or let's look into it with a funciton summary()
```{r}
summary(ols)
```
...
```{r}
ols_edu = lm_robust(prob ~ average_ann_wage + numbEmployed +education, data = jobs)
summary(ols_edu)
```
